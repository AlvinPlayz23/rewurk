...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"glob", "grep", "ls", "sourcegraph", "read"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "crush_info", "crush_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "glob", "ls", "sourcegraph", "todos", "read", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, []string{"glob", "ls", "sourcegraph", "read"}, taskAgent.AllowedTools)
}

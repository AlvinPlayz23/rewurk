...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case completions.CompletionItemsLoadedMsg:
		if m.completionsOpen {
			m.completions.SetItems(msg.Files)
		}
...
func (m *UI) setSessionMessages(msgs []message.Message) tea.Cmd {
	...
	// Load nested tool calls for agent tools.
...
// loadNestedToolCalls recursively loads nested tool calls for agent tools.
...
func (m *UI) handleKeyPressMsg(msg tea.KeyPressMsg) tea.Cmd {
	...
	case uiChat, uiLanding:
		...
		case uiFocusEditor:
			...
			case key.Matches(msg, m.keyMap.Editor.Commands) && strings.TrimSpace(m.textarea.Value()) == "":
				prevHeight := m.textarea.Height()
				m.textarea.SetValue("/")
				m.textarea.MoveToEnd()
				if cmd := m.handleTextareaHeightChange(prevHeight); cmd != nil {
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	case uiLanding:
		m.drawEmptyChat(scr, layout.main)
	...
	case uiChat:
		...
		if m.chatIsEmpty() {
			// Centered, width-capped prompt box.
...
func (m *UI) generateLayout(w, h int) uiLayout {
	...
	case uiLanding:
		// Layout — ChatGPT-style centered prompt when idle.
		//
		// (vertical padding)
		// ------
		...
		// help
		//
		// The greeting and prompt box are grouped and centered vertically.
		uiLayout = m.emptyStateLayout(appRect, uiLayout)

	case uiChat:
		if m.chatIsEmpty() {
			// Empty chat — ChatGPT-style centered prompt layout.
			//
			// (vertical padding)
			// ------
			// main (greeting content)
			...
			// help
			//
			// The greeting and prompt box are grouped and centered
			// vertically. No sidebar, no pills, no compact header.
			uiLayout = m.emptyStateLayout(appRect, uiLayout)
		} else if m.isCompact {
			// Layout
			//
...
// updateStatusData refreshes the status bar data from current UI state.
func (m *UI) updateStatusData() {
	var tokenUsed, tokenMax int
	if m.session != nil {
		tokenUsed = int(m.session.CompletionTokens + m.session.PromptTokens)
	}
	agentCfg := m.com.Config().Agents[config.AgentCoder]
	selectedModel := m.com.Config().Models[agentCfg.Model]
	model := m.com.Config().GetModelByType(agentCfg.Model)
	if model != nil && model.ContextWindow > 0 {
		tokenMax = int(model.ContextWindow)
	}
	...
	if providerConfig, ok := m.com.Config().Providers.Get(providerName); ok {
		providerName = providerConfig.Name
	}
	m.status.SetData(StatusData{
		TokenUsed:    tokenUsed,
		TokenMax:     tokenMax,
		ModelName:    modelName,
...
// chatIsEmpty returns true when the chat has no messages yet, so the UI
// should show the centered prompt layout instead of the full chat layout.
func (m *UI) chatIsEmpty() bool {
	return m.state == uiChat && m.chat.Len() == 0
}

// renderEmptyStateContent builds the greeting block shown above the centered prompt.
func (m *UI) renderEmptyStateContent(width int) string {
	...
	if model != nil {
		...
		if cfg, ok := m.com.Config().Providers.Get(model.ModelCfg.Provider); ok {
			providerName = cfg.Name
		}
	}
	modelLine := lipgloss.NewStyle().
		Foreground(t.Logo.VersionColor).
		Render(fmt.Sprintf("Using %s  ·  %s", modelName, providerName))
...
// drawEmptyChat renders the ChatGPT-style greeting block into the given area.
// The area is sized to the content height (see emptyStateLayout), so the
// block is drawn top-aligned; vertical centering of the greeting plus prompt
// group is handled by the layout.
func (m *UI) drawEmptyChat(scr uv.Screen, mainArea uv.Rectangle) {
	width := mainArea.Dx()
	height := mainArea.Dy()
	if width <= 0 || height <= 0 {
		return
	...
	content := m.renderEmptyStateContent(width)
	...
	uv.NewStyledString(rendered).Draw(scr, mainArea)
}

// maxEmptyEditorWidth caps the width of the centered prompt box in the empty
// (ChatGPT-style) state so it does not span the full terminal width.
const maxEmptyEditorWidth = 80

// emptyStateLayout positions the greeting content and the prompt box as a
// vertically centered group within the given area (ChatGPT-style). The
...
func (m *UI) emptyStateLayout(area image.Rectangle, base uiLayout) uiLayout {
	...
	groupHeight := contentHeight + gap + editorHeight
	topPad := max(0, (area.Dy()-groupHeight)/2)

	mainTop := area.Min.Y + topPad
	mainRect := image.Rect(area.Min.X, mainTop, area.Max.X, mainTop+contentHeight)

	// Center the prompt box horizontally with a capped width.
	editorWidth := min(area.Dx(), maxEmptyEditorWidth)
	editorLeft := area.Min.X + (area.Dx()-editorWidth)/2
	editorTop := mainRect.Max.Y + gap
	editorRect := image.Rect(editorLeft, editorTop, editorLeft+editorWidth, editorTop+editorHeight)

	base.main = mainRect
	base.editor = editorRect
	return base
}
...
func (m *UI) newSession() tea.Cmd {
	...
	return tea.Batch(m.loadPromptHistory(), m.reportCurrentSession(""))
}

...
func TestUpdateLayoutAndSize_EditorGrowthShrinksChat(t *testing.T) {
	...
	// Baseline layout at min textarea height. Add a message so the chat is
	// not empty and uses the docked (non-centered) layout.
	u := newTestUI()
	u.chat.SetMessages(testMessageItem{id: "m-0", text: "hello"})
...
func TestQueuePills(t *testing.T) {
	t.Parallel()

	t.Run("toggle expands for prompt queue", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("does not expand without queue", func(t *testing.T) {
		t.Parallel()
		...
		if u.pillsExpanded {
			t.Fatal("expected pillsExpanded to be false")
		}
	})
}
